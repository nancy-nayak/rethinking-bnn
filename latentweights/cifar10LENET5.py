...
######### Build CNN ###########
cnn = conv_pool_bn(input, 20, (5,5), padding='same', pool_size=(2,2), activation=activation, training=training)
...
for j in range(num_epochs):
    ...
    if acc > old_acc:
        old_acc = acc
        save_path = saver.save(sess, "./cifar10/modelLENET/model.ckpt")
...
saver.restore(sess, "./cifar10/modelLENET/model.ckpt")
...
######### Build CNN ###########
cnn = conv_pool_latent(input, 20, (5,5), padding='same', pool_size=(2,2), activation=activation, training=training)
...
with open('cifar10LENET5.pytraining_accuracy.pkl','rb') as obj:
	# gr = pickle.load(obj)
